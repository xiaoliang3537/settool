...
namespace FileUtils {
	...
	*/
	inline bool DeleteDir(std::string const & sRootDir)
	{
		...
		std::string sDir(sRootDir);
		sDir +="\\*";
        HANDLE hFind = ::FindFirstFile(sDir.c_str(), &wfd);
		...
		do
		{
			...
			{                        
				...
				{
					if(!DeleteDir(sDirName))
						return false;
				}
			}
			else
			{            
				if(::DeleteFile(sDirName.c_str()))
				{
#if FILE_DEBUG
				...
				{
					std::cout<<"fail to delete file: "<<sDirName<<"\t error code:"<<::GetLastError()<<std::endl;
				}
			}
		}
		...
		if(::RemoveDirectory(sRootDir.c_str()))
		{
#if FILE_DEBUG
        ...
        else
        {
            ...
            while( _findnext( ptrf, &localFindData ) == 0 )
            {
                ...
                if(localFindData.attrib == _A_SUBDIR)
                {
                    ...
                    //std::cout<<dir<<"----"<<tempName<<std::endl;
